...
def run_fish_sim(num_pts_L, num_pts_R,num_time_steps, v_x_val, tail_frequency_val, amp_max, num_period, run_opt=False, del_simulator=False):
    #########################################
...
thrust, avg_C_T, simulator = run_fish_sim(num_pts_L=41, num_pts_R=5,num_time_steps=60,
             v_x_val=0.5, tail_frequency_val=0.549, amp_max=0.1, 
...
for i, num_pts_L in enumerate(num_pts_L_list):
    thrust_i, avg_C_T_i = run_fish_sim(num_pts_L=num_pts_L, num_pts_R=5,num_time_steps=num_time_steps,
             v_x_val=0.5, tail_frequency_val=0.5583, amp_max=0.1, 
             num_period=2, run_opt=False, del_simulator=True)
...
for i in range(len(num_pts_L_list)):
    plt.plot(np.linspace(0, np.pi*2, int(num_time_steps/2)), thrust_list[i,int(num_time_steps/2):],label='num_pts_L = {}'.format(num_pts_L_list[i]))
    plt.legend()
...
# # plot thrust
# fig = plt.figure(1)
# for i in range(len(num_pts_R_list)):
#     plt.plot(np.linspace(0, np.pi*2, int(num_time_steps/2)), thrust_list_R[i,int(num_time_steps/2):],label='num_pts_R = {}'.format(num_pts_R_list[i]))
#     plt.legend()
...
for i, num_time_steps in enumerate(num_time_steps_list):
    ...
    thrust_list_time.append(thrust_i.reshape(-1))
    avg_C_T_list_time[i] = avg_C_T_i

# plot thrust
fig = plt.figure(2)
for i in range(len(num_time_steps_list)):
    plt.plot(np.linspace(0, np.pi*2, int(num_time_steps_list[i]/2)), thrust_list_time[i][int(num_time_steps_list[i]/2):],label='num_time_steps = {}'.format(num_time_steps_list[i]))
    plt.legend()
