...
def run_fish_sim(num_pts_L, num_pts_R,num_time_steps, v_x_val, tail_frequency_val, amp_max, num_period, run_opt=False, del_sim=False,problem_name='kin_opt'):
    ...
    ##############################################
    lower = 0.2
    fish_system_model.add_design_variable('tail_frequency',upper=lower*5,lower=lower*0.8)
    fish_system_model.add_design_variable('amplitude_max',upper=lower*2,lower=lower*0.05)
    # add objective
...
run_opt = True
v_x_val = .5
problem_name = 'kin_opt_0922_can_05'
# thrust, avg_C_T, simulator = run_fish_sim(num_pts_L=41, num_pts_R=5,num_time_steps=70,
...
# 0.4
thrust, avg_C_T, simulator = run_fish_sim(num_pts_L=41, num_pts_R=5,num_time_steps=70,
             v_x_val=v_x_val, tail_frequency_val=0.512 , amp_max=0.08, 
            #  v_x_val=v_x_val, tail_frequency_val=0.353, amp_max=0.2, 
             num_period=2, run_opt=run_opt,problem_name=problem_name)
...
if run_opt == True:
    ...
    np.savetxt('results/panel_thrust_power'+case_name,simulator["panel_thrust_power"])
