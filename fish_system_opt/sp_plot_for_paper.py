...
run_opt = False
problem_name = 'sp_full_opt_plots_09'
# amplitude_scalar_0_var = 0.9
amplitude_scalar_0_var = 1
v_x_list = np.array([0.9])
turn_list = np.array([False,])
# ratio_val_dvs = np.array([1.270, 2.167, 1.859, 1.106])
# ratio_val_dvs = np.array([2.220, 2.127, 1.000, 1.026])
ratio_val_dvs = np.array([1.819, 2.018, 1.000, 1.041])
# tail_frequency_list = np.array([1.639,])
# tail_frequency_list = np.array([0.74819,])
# tail_frequency_list = np.array([0.467])
tail_frequency_list = np.array([1.087])
...
a = fish_mp_model.create_input('a_coeff',val=a)
b = fish_mp_model.create_input('b_coeff',val=b)
...
for i in range(len(v_x_list)):
    ...
    if turn == False:
        ...
        fish_mp_model.add_design_variable('ratio',upper=5.,lower=1.)
        fish_mp_model.add_design_variable('amplitude_scalar_0',upper=1.0,lower=0.2)
...
##############################################################################################
amplitue = simulator['fish_system_model_0.amplitude'] * amplitude_scalar_0_var
rigid_fish_mesh = simulator['fish_system_model_0.eel_rigid_mesh'][:,2,0]
np.savetxt('amplitudes/rigid_fish_mesh'+problem_name,amplitue)
np.savetxt('amplitudes/x'+problem_name,rigid_fish_mesh)
plt.plot(rigid_fish_mesh,amplitue)
plt.show()
from mpl_toolkits.mplot3d import Axes3D
import time
