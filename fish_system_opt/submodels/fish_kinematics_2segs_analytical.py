...
class AnalyticalCCMVelocity(csdl.Model):
    ...
    def define(self):
        ...
        for i, var_name in enumerate(seg_names):        
            ...
            alpha_frac = self.create_input(var_name + '_alpha_frac', val=np.linspace(0, 1, num_pts_segments[i]))
...
if __name__ == '__main__':
    ...
    model.create_input('seg1_L', val=np.array([0.2]))
    model.create_input('seg2_L', val=np.array([0.2]))
    ...
    def update(frame):
        for container in [quiver_container1, quiver_container2]:
        ...
        quiver_container1['obj'] = ax.quiver(x1, y1, u1, v1, angles='xy', scale_units='xy', scale=1, color='blue')

        # Segment 2
        x2, y2 = p_seg2[frame, :, 0], p_seg2[frame, :, 1]
        u2, v2 = v_seg2[frame, :, 0], v_seg2[frame, :, 1]
        scatter2.set_data(x2, y2)
        quiver_container2['obj'] = ax.quiver(x2, y2, u2, v2, angles='xy', scale_units='xy', scale=1, color='green')

        return scatter1, scatter2, quiver_container1['obj'], quiver_container2['obj']

    ani = FuncAnimation(fig, update, frames=num_time_steps, interval=10, blit=False)
    plt.show()
