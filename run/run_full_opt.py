...
from fish_system_opt.utils.plots import plot3d
...
#########################################
if run_opt == True:
    fish_system_model.add_design_variable('tail_frequency',upper=1.5,lower=0.1)
    fish_system_model.add_design_variable('v_x',upper=v_x_val,lower=v_x_val)
    fish_system_model.add_design_variable('wave_length',upper=1.5,lower=0.5)
    ...
    fish_system_model.add_design_variable('tail_amplitude',upper=0.4,lower=0.04)
    ...
    prob = CSDLProblem(
        problem_name='full_optimization',
        simulator=simulator,
    )
...
print('tail_amplitude are',simulator['tail_amplitude'])
# print('b coeff is',simulator['b_coeff'])
print('L is',simulator['L'])
print('control_points is',simulator['control_points'])
#########################################
...
def compute_power(sim):
    surface_name = 'eel'
    velocities = sim['eel_kinematic_vel']
    panel_forces_all = - sim['panel_forces_all'] # fish to fluid
    panel_power_x = np.sum(panel_forces_all[:,:,0] * velocities[:,:,0], axis=1)
    panel_power_y = -np.sum(panel_forces_all[:,:,1] * velocities[:,:,1], axis=1)
    panel_power_z = np.sum(panel_forces_all[:,:,2] * velocities[:,:,2], axis=1)
    return panel_power_x, panel_power_y, panel_power_z

def plot_y_vel_of_fish(sim):
    y_vel = sim['eel_coll_vel'][:,:,:,1]
    return y_vel

panel_power_x, panel_power_y, panel_power_z = compute_power(simulator)
y_vel = plot_y_vel_of_fish(simulator)
# plt.figure()
# plt.plot(y_vel[0,:,2])