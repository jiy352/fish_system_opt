...
aluminum = Material(name='custom', E=3E7, G=16.78E6, rho=1150, v=0.49)
...
eel_mesh[:, 1] = np.linspace(-0.1, 0.1, num_beam_nodes)
model.create_input('eel_mesh', val=eel_mesh)  
model.create_input('eel_semi_major_axis', shape=(wing.num_elements), val=0.07)
model.create_input('eel_semi_minor_axis', shape=(wing.num_elements), val=0.03)
...
def update_graph(num):
    ...
    ax.set_title(f'Current time is {num*dt:.1f}')

    # fix the x, y, z range
    ax.set_xlim(-1, 1)
    ax.set_ylim(eel_mesh.min(), eel_mesh.max())
    ax.set_zlim(-eel_mesh.min()*0.3, eel_mesh.min()*0.3)
